        ...
        try {
            ...
            foreach($elements as $el) {
                ...
                if(!is_null($el->getAttribute('data-stat')) && $el->getAttribute('data-stat') === 'college_id'){
                    // $college_el = $el->findElement(WebDriverBy::xpath('//*[@id="roster"]/tbody/tr['.$college_xpath_index.']/td[8]/a'));
                    $college_el = $el->findElements(WebDriverBy::xpath('.//a'));  // $elの直下要素である<a>タグを取得
                ...
                if(!is_null($el->getAttribute('data-stat')) && $el->getAttribute('data-stat') === 'draft_info'){
                    if(is_null($el->getText()) || empty($el->getText())) {
                        ...
                        // $year_el = $el->findElement(WebDriverBy::xpath('//*[@id="roster"]/tbody/tr['.$draftyear_xpath_index.']/td[12]/a'));
            ...
            for($i = 0; $i < count($positions); $i++) {
                ...
                if(is_null($exist)) {
                    ...
                    dump('【'.$urlteam.'】'.$positions[$i].' : '.$firstnamelist[$i].' '.$lastnamelist[$i].' , '.$birthdaylist[$i].' [draft] team: '.$draft_teamlist[$i].', year: '.$draft_yearlist[$i]);
                }
                // rostersデータを作成
